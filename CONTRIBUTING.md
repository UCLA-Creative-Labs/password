...
Thanks for your interest in contributing to Password! ❤️
...
`Password` is configured to use a pipeline to merge changes to `master`. Humans aren't perfect, so we abstracted all our merging to a pipeline managed by a bot, [Mergify](https://mergify.io/). 
